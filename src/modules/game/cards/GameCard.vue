<template>
  ...
  >
    <div v-if="isMultiplier" class="game-card-header" v-bind:style="{color: this.getColorCode(this.color), textAlign: 'center'}">
      <b-iconstack>
    ...
    <div v-else class="game-card-header" v-bind:style="{ color: getColorCode(color) }">
      <span style="position: absolute; left: 10px;">{{value}}</span>
...
button.game-card:disabled:hover {
  cursor: default;
}
